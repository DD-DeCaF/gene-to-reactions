...
def create_app(settings_object):
    ...
    @app.route('/model/modify/genotype')
    def adjust_model():
        if 'model-id' in request.args:
            model = model_by_id(request.args['model-id'])
        elif 'model' in request.args:
            model = cobra.io.json.from_json(request.args['model'])
        else:
            return Response(response="No model is given", status=400)
        return Response(
            response=cobra.io.json.to_json(apply_genotype_changes(model, request.args.getlist('genotype-changes'))),
            status=200,
