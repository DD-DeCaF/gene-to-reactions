.PHONY: setup network build start qa test test-travis license stop clean logs
SHELL:=/bin/bash
...
test-travis:
	...
	docker-compose run --rm $(ci_env) web \
		/bin/sh -c "nosetests tests && codecov"

## Verify source code license headers.
license:
	-./scripts/verify_license_headers.sh genotype_to_model tests
